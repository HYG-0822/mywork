...
import { useNavigate, useSearchParams } from 'react-router-dom';     // 로그인 완료 후 페이지 이동 처리
...
function OAuthCallback() {
    ...
    // ex) xxx?status=success 여기에서 status라는 변수명과 값을 읽어옴
    ...
    // 로딩 상태
    const [isLoading, setIsLoading] = useState(true);   // 기본값을 로딩중으로 설정

    // 중복 실행을 방지하기 위한 flag(상태)
    // react StrictMode가 개발자 모드에서 useEffect룰 두 번 실행하는 것을 방지하기 위해 사용함
    ...
    const handleRequestToken = async () => {
        ...
        // 닫은 데이터를 분석하여 AuthContext에 저장하고,
        // user 정보는 
