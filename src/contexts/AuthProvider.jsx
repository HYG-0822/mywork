...
export function AuthProvider({children}) {
    ...
    // 컴포넌트 마운트되는 시점에서 인증 정보를 복원
    useEffect(()=>{
        const checkAuth = async () => {
            // 1. 로그인 된 상태인지 점검해보고 로그아웃 상태이면 /api/refresh 호출을 생략한다.
            if (user && accessToken) {
                console.log('이미 로그인 된 상태')
                setIsLoading(false);
                return;
            }

            // 2. 로그인 된 이력이 있는지 LocalStorage를 보고 확인해보고 없다면 /api/refresh 호출을 생략한다.
        ...
        checkAuth();
    }, []);  // 두번째 파라미터를 빈 배열로 두면 컴포넌트 마운트시에 콜백 함수가 한번 실행된다.
